...
function AddNewVehicle() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      try {
        await sendRequest();
        toast.success("Vehicle record added successfully."); // Show success toast
        setErrors({});
        setTimeout(() => {
          navigate("/comparisonhome");
        }, 2000);
      } catch (error) {
        toast.error("Error adding vehicle record."); // Show error toast
      }
    }
  };
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      throw new Error("Error uploading vehicle");    // Throw an error for the catch in handleSubmit
    }
  };
  ...
  return (
    <div className="vcaddform-container">
      ...
      {message && (
        <div className="vcmessage" style={{ color: message.includes("Error") ? "red" : "green" }}>
          <h2>{message}</h2>
        </div>
      )}
    </div>
